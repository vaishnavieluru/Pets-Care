...
const Login = () => {
  ...
  const handleSendCode = () => {
    if(userRole){
        ...
        toast.error('Select the your role');
  ...
  const handleVerifyCode = async () => {
    ...
    signInWithCredential(auth, credential)
      .then(async (result) => {
        ...
        const docRef = doc(db, userRole, result.user.uid);
        const documentSnapshot = await getDoc(docRef);
  ...
  const handleSubmit = async () => {
    if(name && validateEmail(email)){
      const docRef = doc(db, userRole, currentUser.uid);
  ...
  return (
    ...
    <Card className={classes.card}>
      ...
      variant="contained"
        ...
        color={userRole === 'doctor' ?  'secondary' : 'x' }
      >
        Doctor
      </Button>
      <Button
      variant="contained"
        ...
        color={userRole === 'supplier' ?  'secondary' : 'x' }
      >
        Supplier
      </Button>
    </div>
